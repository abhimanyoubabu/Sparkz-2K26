...
export default function ParticleImage(props: ParticleImageProps) {
  ...
  const {
    ...
    singleColor = "#2E2645",
    multiColors = ["#2E2645", "#4A3F6B", "#C9A227", "#B8860B", "#1E1B2E"],
    hoverEnabled = true,
    hoverConfig = {},
    repulsionEnabled = true,
    repulsionConfig = {},
  ...
  const image = imageConfig?.image || "/sparkz-logo.png";
  ...
  const startAnimRef = useRef<((newState: string) => void) | null>(null);
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        if (pColor === "single") {
          ...
          const cols = (
            mcColors || ["#2E2645", "#4A3F6B", "#C9A227", "#B8860B", "#1E1B2E"]
          ).filter(Boolean);
          if (cols.length > 0) {
          ...
          /*
           * Exact visual direction from the uploaded Sparkz logo:
           * warm champagne-gold highlights, rich antique-gold midtones,
           * and deeper bronze-gold shadows.
           */
          const lum = (0.299 * dr + 0.587 * dg + 0.114 * db) / 255;

          // Contrast curve keeps the metallic gold visible instead of dull.
          let t = (lum - 0.10) / 0.88;
          t = Math.max(0, Math.min(1, t));
          t = t * t * (3 - 2 * t);

          // Palette sampled/approximated from the uploaded logo:
          // highlight #F9E9BE → light gold #EAD39A → gold #CBA25F
          // → bronze gold #A8783E → deep bronze #704A24
          const stops = [
            { t: 0.00, r: 112, g: 72, b: 34 }, // deep bronze
            { t: 0.22, r: 168, g: 122, b: 62 }, // bronze gold
            { t: 0.48, r: 203, g: 162, b: 95 }, // warm gold
            { t: 0.72, r: 234, g: 211, b: 154 }, // light champagne gold
            { t: 1.00, r: 249, g: 233, b: 190 }, // bright highlight
          ];
          ...
          const range = Math.max(0.0001, right.t - left.t);
          const f = (t - left.t) / range;

          dr = Math.round(left.r + (right.r - left.r) * f);
          dg = Math.round(left.g + (right.g - left.g) * f);
          db = Math.round(left.b + (right.b - left.b) * f);

          // Small saturation boost for the same rich metallic-gold appearance.
          const maxC = Math.max(dr, dg, db);
          const minC = Math.min(dr, dg, db);
          const mid = (maxC + minC) / 2;
          const satBoost = 1.08;

          if (maxC !== minC) {
            dr = Math.round(Math.max(0, Math.min(255, mid + (dr - mid) * satBoost)));
            dg = Math.round(Math.max(0, Math.min(255, mid + (dg - mid) * satBoost)));
            db = Math.round(Math.max(0, Math.min(255, mid + (db - mid) * satBoost)));
          }

          // Keep particles strong and luminous.
          da = Math.min(255, Math.round(da * 1.08));
        }
  ...
  // Auto-cycle effect: periodically assemble logo and scatter/roam when not hovering
  useEffect(() => {
    if (!autoCycle) return;
    ...
    const triggerAssembleAndDisperse = () => {
      ...
      holdTimer = setTimeout(() => {
        if (!mouseRef.current.active && startAnimRef.current) {
          startAnimRef.current("scattering");
        }
      }, holdDuration);
    };

    // Initial trigger after 1.2 seconds, then recurring
